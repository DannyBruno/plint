...
- **OpenAI, [_Prompt guidance_](https://developers.openai.com/api/docs/guides/prompt-guidance)** — corroborates several Anthropic recommendations (especially the warning against absolute language like `MUST` / `NEVER` for judgment calls — see `PROMPT005`) and adds outcome-first, stopping-condition, and output-contract guidance. Note one mild disagreement: OpenAI does **not** require XML structuring (plain headers work), so if you're targeting OpenAI models you may want to suppress `PROMPT008` in `.plint.toml`.
